...
php -r "file_put_contents('.env', preg_replace('/^DB_PORT=.*/m','DB_PORT=35060', file_get_contents('.env')));"
php -r "file_put_contents('.env', preg_replace('/^DB_DATABASE=.*/m','DB_DATABASE=miniliga', file_get_contents('.env')));"
php -r "file_put_contents('.env', preg_replace('/^DB_USERNAME=.*/m','DB_USERNAME=doadmin', file_get_contents('.env')));"
php -r "file_put_contents('.env', preg_replace('/^DB_PASSWORD=.*/m','DB_PASSWORD=A89rKShc1XdYgWthIa', file_get_contents('.env')));"

php -m | grep -qi pdo_mysql || echo '⚠️ Habilita pdo_mysql en tu php.ini'
